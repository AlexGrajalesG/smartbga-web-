        ...
        <div className="max-w-6xl mx-auto px-4 h-16 flex items-center justify-between">
          ...
          <div className="flex items-center gap-1">
            ...
            <Link href="/carrito" className="relative p-2 cursor-pointer" aria-label="Carrito">
              <ShoppingBag
                size={22}
                className={`text-neutral-700 hover:text-[#8C1A1A] transition-colors ${bouncing ? 'cart-bounce' : ''}`}
                style={{ animation: bouncing ? 'cart-bounce 420ms cubic-bezier(0.36,0.07,0.19,0.97)' : 'none' }}
              />
              {mounted && totalCarrito > 0 && (
                <span className="absolute -top-0.5 -right-0.5 bg-[#8C1A1A] text-white text-[10px] font-bold w-4 h-4 rounded-full flex items-center justify-center tabular-nums">
                  {totalCarrito}
                </span>
              )}
            </Link>
